 ...
 * @param activeItem
 */
export const handleURLQueries = (asPath: string, query: any, path: string | undefined): boolean => {
  if (Object.keys(query).length && path) {
    const arr = Object.keys(query);

    return asPath.includes(path) && asPath.includes(query[arr[0]] as string) && path !== '/';
  }
